...
namespace MusicalNoteLauncher.Utils
{
    public class EasyTierManager
    {
        ...
        public async Task<bool> DownloadCoreAsync(IProgress<int> progress = null)
        {
            ...
            try
            {
                OnLogOutput?.Invoke("正在下载陶瓦核心...");
                using (var client = new HttpClient())
                {
                    using (var response = await client.GetAsync("https://github.com/terracotta-network/terracotta/releases/latest/download/terracotta-windows-x86_64.exe", HttpCompletionOption.ResponseHeadersRead))
                    {
            ...
            {
                OnLogOutput?.Invoke($"下载失败：{ex.Message}");
                return false;
            }
        }

        public (string, string) GenerateNetworkCredentials()
        {
            var random = new Random();
            char roomLetter = (char)random.Next('A', 'Z' + 1);
            return ($"{roomLetter}", GenerateInviteCode());
        }
        ...
        public bool StartClient(string networkName, string networkSecret)
        {
            if (!IsCoreInstalled()) { OnLogOutput?.Invoke("陶瓦核心未安装"); return false; }
            if (_isRunning) Stop();
            try
            {
                ...
                var args = $"client --name \"{networkName}\" --password \"{networkSecret}\"";
                OnLogOutput?.Invoke($"正在连接房间：{networkName}");
                var psi = new ProcessStartInfo { FileName = CoreExePath, Arguments = args, WorkingDirectory = TerracottaDir, CreateNoWindow = true, UseShellExecute = false, RedirectStandardOutput = true, RedirectStandardError = true };
                _coreProcess = new Process { StartInfo = psi };
                _coreProcess.OutputDataReceived += (s, e) => { if (!string.IsNullOrEmpty(e.Data)) OnLogOutput?.Invoke(e.Data); };
                _coreProcess.ErrorDataReceived += (s, e) => { if (!string.IsNullOrEmpty(e.Data)) OnLogOutput?.Invoke($"[ERROR] {e.Data}"); };
        ...
        public NetworkStatus GetNetworkStatus() => null;
        public string GetPeers() => "Terracotta 不支持此功能";
    ...
    public class NetworkStatus { public string VirtualIP { get; set; } public bool IsConnected { get; set; } }
}
