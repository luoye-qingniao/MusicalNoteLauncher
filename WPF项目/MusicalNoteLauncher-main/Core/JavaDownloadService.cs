...
namespace MusicalNoteLauncher.Core
{
    public class JavaDownloadService
    {
        ...
        // Java 下载镜像源（BMCLAPI 主源，Adoptium API 备用）
        // BMCLAPI 版本列表: https://bmclapi2.bangbang93.com/java/list
        private readonly List<string> _javaDownloadUrls = new List<string>
        {
        ...
        private List<string> GetDownloadUrls(int javaVersion)
        {
            ...
            foreach (string urlTemplate in _javaDownloadUrls)
            {
                // Adoptium API 格式: {0} 占位符替换为版本号
                if (urlTemplate.Contains("{0}"))
                {
                    urls.Add(string.Format(urlTemplate, javaVersion));
                }
                else
                {
                    // BMCLAPI 格式: {baseUrl}/java-{version}-{os}.zip
                    urls.Add($"{urlTemplate}/java-{javaVersion}-{os}.zip");
                }
            }
        ...
        {
            return $"Oracle JDK 镜像 ({index + 1}/{total})";
        }
        ...
        public async Task<string> DownloadMojangRuntimeAsync(string javaVersion, DownloadProgress progress = null, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                int downloaded = 0;
                int threadCount = Math.Min(8, total); // 小文件不需要太多线程，8个并发足够
                    ...
                    var task = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            foreach (string url in urls)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    {
                                        LogReceived?.Invoke($"下载失败: {relativePath} ({ex.Message})");
                                    }
                                }
                            }
                        }
                        finally
                        {
                            ...
                            progress?.Report(new DownloadProgressInfo
                            {
                                ...
                                Status = $"下载运行时文件 ({current}/{total})"
                            });
                            StatusChanged?.Invoke($"下载运行时文件 ({current}/{total})");
                        }
                    }, cancellationToken);
