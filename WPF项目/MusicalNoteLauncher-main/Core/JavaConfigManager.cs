...
namespace MusicalNoteLauncher.Core
{
    public class JavaConfigManager
    {
        ...
        public List<DetectedJava> DetectInstalledJava()
        {
            ...
            {
                // MNL 自安装 Java 目录（优先级最高）
                Path.Combine(MNLEnvironment.MinecraftPath, "java"),
                // MNL 自安装 Mojang 运行时
                Path.Combine(MNLEnvironment.MinecraftPath, "runtime"),
        ...
        public int GetRecommendedJavaVersion(string minecraftVersion)
        {
            try
            {
                ...
                if (parts.Length >= 2)
                {
                    ...
                    // MC 1.22+ / 2.x+ 推荐 Java 25
                    if (major > 1 || (major == 1 && minor >= 22))
                    {
                        return 25;
                    }
                    // 1.20.5 - 1.21.x 推荐 Java 21
                    else if (major > 1 || (major == 1 && minor > 20) || (major == 1 && minor == 20 && patch >= 5))
                    {
                        return 21;
                    }
