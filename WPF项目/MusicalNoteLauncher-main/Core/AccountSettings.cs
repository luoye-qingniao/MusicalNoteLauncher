    ...
    public static class Settings
    {
        ...
        private static void Load()
        {
            lock (_lock)
            {
                ...
                {
                    // JSON 损坏时创建备份，防止空数据覆盖
                    try
                    {
                        string backupPath = FilePath + ".bak";
                        if (File.Exists(FilePath))
                            File.Copy(FilePath, backupPath, true);
                        System.Diagnostics.Debug.WriteLine($"[AccountSettings] 配置文件损坏，已备份至 {backupPath}: {ex.Message}");
                    }
                    catch { }
                    _store = new Dictionary<string, object>();
                }
        ...
        public static void Save()
        {
            lock (_lock)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[AccountSettings] 保存失败: {ex.Message}");
                }
            }
        }
