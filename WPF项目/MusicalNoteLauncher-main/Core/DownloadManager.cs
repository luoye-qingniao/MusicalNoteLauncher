...
namespace MusicalNoteLauncher.Core
{
    ...
    public static class DownloadManager
    {
        ...
        /// <summary>
        /// 添加下载任务到下载中心（任务管理器）并开始下载（使用自定义完整路径）
        /// </summary>
        public static bool AddDownloadTask(string resourceName, string savePath, DownloadVersionInfo version, string displayFileName = null)
        {
            try
            {
                ...
                var task = new GenericDownloadTaskViewModel(version.DownloadUrl, savePath, displayFileName ?? Path.GetFileName(savePath));
        ...
        /// 构建默认保存路径（目标目录 + 版本文件名）
        /// </summary>
        public static string BuildSavePath(string resourceName, string targetDir, DownloadVersionInfo version)
        {
        ...
        public static async Task<bool> QuickDownload(string resourceName, string targetDir, string modrinthId, long? curseForgeId = null)
        {
            ...
            bool success = AddDownloadTask(resourceName, BuildSavePath(resourceName, targetDir, recommended), recommended);
            if (success)
            {
