...
namespace MusicalNoteLauncher.Core
{
    public class CurseForgeApiService
    {
        ...
        /// 获取单个模组详细信息（名称、图标）
        /// </summary>
        public async Task<CurseForgeModDetail> GetMod(long modId)
        {
            try
            {
                string url = $"{BaseUrl}/mods/{modId}";
                var response = await _httpClient.GetAsync(url);
                if (!response.IsSuccessStatusCode)
                    return null;
                var content = await response.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<CurseForgeModDetail>(content);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"CurseForge GetMod Exception: {ex.Message}");
                return null;
            }
        }
        ...
        /// 批量获取多个模组详细信息
        /// </summary>
        public async Task<List<CurseForgeModDetail>> GetMods(List<long> modIds)
        {
            var results = new List<CurseForgeModDetail>();
            try
            {
                if (modIds == null || modIds.Count == 0)
                    return results;

                string idsParam = "[" + string.Join(",", modIds) + "]";
                string url = $"{BaseUrl}/mods";
                var payload = new StringContent(
                    System.Text.Json.JsonSerializer.Serialize(new { modIds = modIds }),
                ...
                if (!response.IsSuccessStatusCode)
                {
                    System.Diagnostics.Debug.WriteLine($"CurseForge GetMods Error: {response.StatusCode}");
                    return results;
                }

                var content = await response.Content.ReadAsStringAsync();
                var result = System.Text.Json.JsonSerializer.Deserialize<CurseForgeModsResponse>(content);
                if (result?.Data != null)
                    results.AddRange(result.Data);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"CurseForge GetMods Exception: {ex.Message}");
            }
            return results;
        }
    }
    ...
    public class CurseForgeDependency
    {
        ...
        /// <summary>是否为必需依赖（relationType=3）</summary>
        public bool IsRequired => RelationType == 3;
    }

    /// <summary>
    /// CurseForge 模组详细信息
    /// </summary>
    public class CurseForgeModDetail
    {
        [JsonPropertyName("data")]
        public CurseForgeModDetailData Data { get; set; }

        public string Name => Data?.Name;
        public string LogoUrl => Data?.Logo?.ThumbnailUrl ?? Data?.Logo?.Url;
    }

    public class CurseForgeModDetailData
    {
        [JsonPropertyName("id")]
        public long Id { get; set; }
    ...
    /// <summary>
    /// CurseForge 批量查询模组的响应
    /// </summary>
    public class CurseForgeModsResponse
    {
        [JsonPropertyName("data")]
        public List<CurseForgeModDetail> Data { get; set; }
    }
}