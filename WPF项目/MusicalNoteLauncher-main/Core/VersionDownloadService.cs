    ...
    public class VersionDownloadService
    {
        ...
        public async Task<DownloadTaskInfo> StartDownloadAsync(VersionInfo version, DownloadProgress progress, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                await DownloadWithBmclApiFallbackAsync(
                    version.Url, version.Url.Replace("bmclapi2.bangbang93.com", "launchermeta.mojang.com"),
                    jsonFile, progress, cancellationToken, 0, 5, "版本JSON");
                ...
                await DownloadWithBmclApiFallbackAsync(
                    jarUrl, jarUrl.Replace("bmclapi2.bangbang93.com", "s3.amazonaws.com"),
                    jarFile, progress, cancellationToken, 5, 25, "客户端JAR");
                ...
                foreach (var lib in libraries)
                {
                    ...
                    if (!string.IsNullOrEmpty(lib.Url) && !string.IsNullOrEmpty(lib.Path))
                    {
                        ...
                        string mojangUrl = lib.Url.Replace("bmclapi2.bangbang93.com", "libraries.minecraft.net");
                        await DownloadWithBmclApiFallbackAsync(
                            lib.Url, mojangUrl, lib.Path, progress, cancellationToken, segStart, segEnd, "依赖库");
                    }
                    libIndex++;
                ...
                if (natives.Count > 0)
                {
                    ...
                    foreach (var native in natives)
                    {
                        ...
                        if (!string.IsNullOrEmpty(native.Url) && !string.IsNullOrEmpty(native.Path))
                        {
                            ...
                            string mojangUrl = native.Url.Replace("bmclapi2.bangbang93.com", "libraries.minecraft.net");
                            await DownloadWithBmclApiFallbackAsync(
                                native.Url, mojangUrl, native.Path, progress, cancellationToken, segStart, segEnd, "Natives库");
                        }
                        nativeIndex++;
                    }
                ...
                if (!string.IsNullOrEmpty(assetIndex.Url) && !string.IsNullOrEmpty(assetIndex.Path))
                {
                    ...
                    string assetIndexFallbackUrl = assetIndex.Url.Contains("bmclapi")
                        ? assetIndex.Url.Replace("bmclapi2.bangbang93.com", "launchermeta.mojang.com")
                        : assetIndex.Url.Replace("launchermeta.mojang.com", "bmclapi2.bangbang93.com");
                    await DownloadWithBmclApiFallbackAsync(
                        assetIndex.Url, assetIndexFallbackUrl,
                        assetIndex.Path, progress, cancellationToken, 65, 67, "资源索引");
        ...
        private async Task DownloadFileWithProgressAsync(string url, string savePath, int startProgress, int endProgress)
        {
            ...
            for (int i = 0; i < retryCount; i++)
            {
                try
                {
                    ...
                    StatusChanged?.Invoke($"下载完成: {Path.GetFileName(savePath)}");
                    return;
                }
                catch (Exception ex) when (!(ex is OperationCanceledException))
                {
                    ...
                    if (i == retryCount - 1)
                    {
                        Logger.Error($"[下载] {Path.GetFileName(savePath)} 下载失败（已重试 {retryCount} 次）: {ex.Message}");
                        throw;
                    }

                    Logger.Warning($"[下载] {Path.GetFileName(savePath)} 下载失败，{delayMs}ms 后重试 ({i + 1}/{retryCount}): {ex.Message}");
        ...
        /// <summary>BMCLAPI 下载失败时自动回退到 Mojang 官方源</summary>
        private async Task DownloadWithBmclApiFallbackAsync(Func<Task> bmclApiDownload, Func<Task> mojangDownload, string fileDesc)
        {
            try
            {
                await bmclApiDownload();
            }
            catch (Exception ex) when (!(ex is OperationCanceledException))
            {
                Logger.Warning($"[下载] BMCLAPI {fileDesc} 下载失败，回退到 Mojang 官方源: {ex.Message}");
                await mojangDownload();
            }
        }

        /// <summary>BMCLAPI 下载失败时自动回退到 Mojang 官方源（带 CancellationToken）</summary>
        private async Task DownloadWithBmclApiFallbackAsync(
            string bmclApiUrl, string mojangUrl, string savePath,
            DownloadProgress progress, CancellationToken cancellationToken,
            int startProgress, int endProgress, string fileDesc)
        {
            try
            {
                await DownloadFileWithProgressAsync(bmclApiUrl, savePath, progress, cancellationToken, startProgress, endProgress);
            }
            catch (Exception ex) when (!(ex is OperationCanceledException))
            {
                Logger.Warning($"[下载] BMCLAPI {fileDesc} 下载失败，回退到 Mojang 官方源: {ex.Message}");
                await DownloadFileWithProgressAsync(mojangUrl, savePath, progress, cancellationToken, startProgress, endProgress);
            }
        }
        ...
        private async Task DownloadLibrariesAsync(List<LibraryDownloadInfo> libraries, int startProgress, int endProgress)
        {
            ...
            int threadCount = Math.Min(SettingsManager.Settings.DownloadThreads, totalFiles);
                ...
                var task = Task.Run(async () =>
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(lib.Url) && !string.IsNullOrEmpty(lib.Path))
                        {
                            ...
                            {
                                // BMCLAPI 失败，尝试 Mojang 官方源
                                string mojangUrl = lib.Url.Replace("bmclapi2.bangbang93.com", "libraries.minecraft.net");
                                Logger.Warning($"[库] BMCLAPI 失败，回退到 Mojang 官方源: {ex.Message}");
                                await DownloadFileAsync(mojangUrl, lib.Path, _cts.Token);
                            }
                        }
                    }
                    finally
                    {
                        ...
                        double progressValue = startProgress + (double)current / totalFiles * (endProgress - startProgress);
                        ProgressChanged?.Invoke((int)progressValue);
                        StatusChanged?.Invoke($"[库] {current}/{totalFiles}");
                    }
                }, _cts.Token);
            ...
            foreach (var native in natives)
            {
                ...
                var task = Task.Run(async () =>
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(native.Url) && !string.IsNullOrEmpty(native.Path))
                        {
                            ...
                            catch (Exception ex) when (!(ex is OperationCanceledException))
                            {
                                // BMCLAPI 失败，尝试 Mojang 官方源
                                string mojangUrl = native.Url.Replace("bmclapi2.bangbang93.com", "libraries.minecraft.net");
                                Logger.Warning($"[Natives] BMCLAPI 失败，回退到 Mojang 官方源: {ex.Message}");
                                await DownloadFileAsync(mojangUrl, native.Path, _cts.Token);
                            }
                        }
                    }
                    finally
                    {
                        ...
                        double progressValue = startProgress + (double)current / totalFiles * (endProgress - startProgress);
                        ProgressChanged?.Invoke((int)progressValue);
                        StatusChanged?.Invoke($"[Natives] {current}/{totalFiles}");
                    }
                }, _cts.Token);
        ...
        public async Task StartDownloadAsync()
        {
            ...
            await DownloadWithBmclApiFallbackAsync(
                () => DownloadFileWithProgressAsync(_versionInfo.Url, jsonFile, 0, 5),
                () => DownloadFileWithProgressAsync(_versionInfo.Url.Replace("bmclapi2.bangbang93.com", "launchermeta.mojang.com"), jsonFile, 0, 5),
                "版本JSON");
            ...
            if (!string.IsNullOrEmpty(assetIndex.Url) && !string.IsNullOrEmpty(assetIndex.Path))
            {
                StatusChanged?.Invoke($"正在下载资源索引...");
                await DownloadWithBmclApiFallbackAsync(
                    () => DownloadFileWithProgressAsync(assetIndex.Url, assetIndex.Path, 55, 60),
                    () => DownloadFileWithProgressAsync(assetIndex.Url.Replace("bmclapi2.bangbang93.com", "launchermeta.mojang.com"), assetIndex.Path, 55, 60),
                    "资源索引");

                StatusChanged?.Invoke($"正在下载资源文件...");
                int assetCount = await DownloadAssetsAsync(assetIndex.Path, _cts.Token);
            ...
            await DownloadWithBmclApiFallbackAsync(
                () => DownloadFileWithProgressAsync(jarUrl, jarFile, 75, 100),
                () => DownloadFileWithProgressAsync(jarUrl.Replace("bmclapi2.bangbang93.com", "s3.amazonaws.com"), jarFile, 75, 100),
                "客户端JAR");
                    ...
                    if (doc.RootElement.TryGetProperty("objects", out JsonElement objects))
                    {
                        ...
                        foreach (var obj in objects.EnumerateObject())
                        {
                            ...
                            if (!string.IsNullOrEmpty(hash))
                            {
                                ...
                                {
                                    // 校验已有文件的 SHA-1，不匹配则重新下载
                                    if (VerifyFileHash(assetPath, hash))
                                    {
                                        needsDownload = false;
                                    }
                                    else
                                    {
                                        Logger.Warning($"[Assets] 资源文件 {subdir}/{hash} 校验失败，将重新下载");
                        ...
                        foreach (var asset in assetList)
                        {
                            ...
                            var task = Task.Run(async () =>
                            {
                                try
                                {
                                    ...
                                    // 构建下载源列表：BMCLAPI 优先，Mojang 官方源作为后备
                                    var urls = new List<string>();
                                    ...
                                    foreach (var url in urls)
                                    {
                                        ...
                                        {
                                            Logger.Warning($"[Assets] 从 {new Uri(url).Host} 下载失败: {ex.Message}，尝试下一个源");
                                        }
                                    }
                                    
                                    if (!downloaded)
                                    {
                                        Logger.Error($"[Assets] 所有下载源均失败: {subdir}/{hash}");
                                        return;
                                    }
                                    
                                    // 下载后校验 SHA-1
                                    if (!VerifyFileHash(asset.path, hash))
                                    {
                                        Logger.Error($"[Assets] 资源文件 {subdir}/{hash} 下载后校验失败");
                                        try { File.Delete(asset.path); } catch { }
                                        return;
                                    }
