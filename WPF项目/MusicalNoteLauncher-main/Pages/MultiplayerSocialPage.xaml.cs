...
namespace MusicalNoteLauncher.Pages
{
		public partial class MultiplayerSocialPage : UserControl
	{
		...
		private async void BtnDownloadTaoWaCore_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				btnDownloadTaoWaCore.IsEnabled = false;
				TaoWaDownloadProgress.Visibility = Visibility.Visible;
				TaoWaDownloadProgress.Value = 0;
				TaoWaDownloadStatus.Visibility = Visibility.Visible;
				TaoWaDownloadStatus.Text = "正在下载陶瓦核心...";

				var progress = new Progress<int>(p =>
				{
					Dispatcher.Invoke(() =>
					{
						TaoWaDownloadProgress.Value = p;
						TaoWaDownloadStatus.Text = $"正在下载陶瓦核心... {p}%";
					});
				});

				bool success = await _easyTier.DownloadCoreAsync(progress);

				Dispatcher.Invoke(() =>
				{
					if (success)
					{
						TaoWaDownloadStatus.Text = "下载完成！";
						RefreshTaoWaCoreStatus();
					}
					else
					{
						TaoWaDownloadStatus.Text = "下载失败，请检查网络连接后重试";
						btnDownloadTaoWaCore.IsEnabled = true;
					}
					TaoWaDownloadProgress.Visibility = Visibility.Collapsed;
				});
			}
			catch (Exception ex)
			{
				Dispatcher.Invoke(() =>
				{
					TaoWaDownloadStatus.Text = $"下载出错：{ex.Message}";
					btnDownloadTaoWaCore.IsEnabled = true;
					TaoWaDownloadProgress.Visibility = Visibility.Collapsed;
				});
			}
		...
		private void RefreshTaoWaCoreStatus()
		{
			if (_easyTier.IsCoreInstalled())
			{
				TaoWaCoreNotInstalled.Visibility = Visibility.Collapsed;
				TaoWaCoreInstalled.Visibility = Visibility.Visible;
			}
			else
			{
				TaoWaCoreNotInstalled.Visibility = Visibility.Visible;
				TaoWaCoreInstalled.Visibility = Visibility.Collapsed;
			}
		}
		...
		private async void BtnCreateRoom_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				if (!hasInternet)
				{
					ModernMessageBox.ShowWarning("未检测到互联网连接\n\n陶瓦联机需要互联网才能创建房间\n请检查网络连接后重试", "网络不可用");
					return;
				}

				// 检测 Minecraft 是否已开放局域网世界及端口号
				int detectedPort = this.DetectMinecraftLanPort();
				if (detectedPort <= 0)
				{
					ModernMessageBox.ShowWarning(
						"未检测到已开放的 Minecraft 局域网世界\n\n" +
						"请先在 Minecraft 中:\n" +
						"1. 进入单人存档\n" +
						"2. 按 ESC →\"对局域网开放\"\n" +
						"3. 点击\"创建一个局域网世界\"\n\n" +
				...
				int taoWaPort = detectedPort;
				string networkName = this.GenerateRandomCode();
				string networkSecret = this.GenerateRandomCode();
				if (this._easyTier.StartHost(networkName, networkSecret, taoWaPort))
				{
					this._currentInviteCode = string.Format("P{0:X4}-{1}-{2}-02000", taoWaPort, networkName, networkSecret);
					this.txtInviteCodeDisplay.Text = this._currentInviteCode;
					this.txtPortDisplay.Text = taoWaPort.ToString();

					// 检测端口可访问性, 提示端口转发
					string portInfo = CheckPortAccessibility(taoWaPort);
					this.txtPortInfo.Text = portInfo;
					this.txtPortInfo.Text += $"\n\n检测到 Minecraft 局域网已开放，端口: {taoWaPort}";
		...
		private async void BtnJoinRoom_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				else
				{
					...
					{
						// 邀请码格式: P{Port:X4}-{Name}-{Secret}-02000
						// 正确提取端口(第 1 组去掉 P 前缀)、房间名(第 2 组)、密码(第 3 组)
						string[] parts = text2.Split('-');
						if (parts.Length < 3)
						{
							ModernMessageBox.ShowWarning("邀请码格式不正确", "邀请码无效");
							return;
						}
						string networkName = parts[1];
						string networkSecret = parts[2];
						int port;
						try
						{
							port = int.Parse(parts[0].Substring(1), NumberStyles.HexNumber);
						}
						catch
						{
							ModernMessageBox.ShowWarning("邀请码中的端口号无效", "邀请码无效");
							return;
						}
						if (this._easyTier.StartClient(networkName, networkSecret, port))
						{
		...
		private async void BtnDownloadMCTier_Click(object sender, RoutedEventArgs e)
		{
			...
			string[] apiMirrors = new[]
			{
				...
				"https://gh.con.sh/https://api.github.com/repos/pmh1314520/MCTier/releases/latest",
				"https://gh.api.99988866.xyz/https://api.github.com/repos/pmh1314520/MCTier/releases/latest",
			};
			...
			using (var apiClient = new HttpClient())
			{
				...
				foreach (string apiUrl in apiMirrors)
				{
					...
					{
						Debug.WriteLine($"[MCTier] API镜像失败 {apiUrl}: {ex.Message}");
					}
				}
			}
			...
			foreach (string mirrorUrl in downloadMirrors)
			{
				...
				{
					Debug.WriteLine($"[MCTier] 下载镜像失败 {new Uri(mirrorUrl).Host}: {ex.Message}");
				}
			}
		...
		/// 共享随机数生成器, 避免连续调用时因 TickCount 种子相同而产生相同输出
		...
		private string GenerateRandomCode()
		{
			...
			lock (_rng)
			{
				for (int i = 0; i < 5; i++)
				{
					array[i] = "0123456789ABCDEFGHJKLMNPQRSTUVWXYZ"[_rng.Next("0123456789ABCDEFGHJKLMNPQRSTUVWXYZ".Length)];
				}
			}
		...
		/// 检测互联网连接是否可用
		/// </summary>
		private async System.Threading.Tasks.Task<bool> CheckInternetAsync()
		{
			try
			{
		...
		/// <summary>
		/// 检测指定端口是否可能被外部访问
		/// 使用 UPnP 检测路由器是否支持端口映射
		/// </summary>
		private string CheckPortAccessibility(int port)
		{
			try
			{
				// 检测 NAT 类型: 尝试 STUN 检测 (简化版)
				using (var client = new System.Net.Http.HttpClient { Timeout = TimeSpan.FromSeconds(5) })
				{
					// 通过外部服务获取公网 IP, 间接判断是否有互联网访问
					var response = client.GetStringAsync("https://api.ipify.org").Result;
					if (!string.IsNullOrEmpty(response))
					{
						return $"公网 IP: {response}\n端口 {port} 需要路由器端口转发后才能被外网访问\n\n请在路由器中设置端口转发:\n内部端口 {port} → 外部端口 {port}";
					}
				}
			}
			catch { }
			return $"端口 {port} 可能无法从外网访问\n\n请检查路由器UPnP是否开启,\n或手动设置端口转发: {port} → {port}";
		}
	}
}
