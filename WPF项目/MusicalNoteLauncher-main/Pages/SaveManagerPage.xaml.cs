...
namespace MusicalNoteLauncher.Pages
{
		public partial class SaveManagerPage : UserControl
	{
		...
		public SaveManagerPage()
		{
			...
			{
				ModernMessageBox.ShowError("加载存档管理页面失败：" + ex.Message, "存档管理错误");
			}
		}
		...
		private void LoadSaves()
		{
			...
			// 验证游戏路径是否有效（防止 Roaming.minecraft 之类缺少分隔符的路径）
			if (!Directory.Exists(path))
			{
				this.lstSaves.Items.Add(new SaveManagerPage.SaveItem
				{
					SaveName = "⚠ 游戏目录不存在",
					SaveInfo = "请在设置中检查 Minecraft 路径:\n" + path,
					PlayTime = "",
					FolderPath = "",
					FileSize = 0L
				});
